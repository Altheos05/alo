...
| D84 | **Sujets de service.** Des ~30 verbes dédiés de la §21 `whatsapp_commands_list.md` (services de Capitale Alne, un par PNJ), **~21 seulement** deviennent des sujets `!demander [NPC] [sujet]` (K0/K1, jamais K2/K3 — un service ne se débloque pas, il se rend) déclenchant leur primitive `SYS_*` déjà existante via le contrat de mutation standard (D-DET-2). **13 gardent une commande dédiée** : 3 mécaniques à point d'accès multiple (réputation raciale, sertissage de gemme, tutoriel) + **10 archétypes déjà répliqués dans 2 à 5 autres villes** (`!voyage`, `!raid_register`/`!raid_join`, `!mount_rent`, `!sharpen`, `!fence`, `!oracle`, `!memorial`, `!laundry`, `!loan`, `!heal` — trouvés par vérification croisée *après* le diagnostic initial, qui n'avait regardé que le roster d'Alne isolément). 2 verbes (`!buy_info`/`!buy_silence`) retirés — purement redondants avec le K2 `PAY:<N>` déjà existant. | Diagnostic UX étape 54 : interface superficielle (1 verbe = 1 PNJ) là où `!demander` résolvait déjà toute l'enveloppe QI d'un PNJ derrière un seul verbe. Correction en cours de route : la moitié des verbes visés étaient en fait déjà des archétypes multi-villes (deux adaptateurs = un seam réel) — les convertir aurait désynchronisé Alne du reste du monde au lieu d'améliorer l'interface |
| D83 | **Menu contextuel numéroté** (diagnostic UX PE : ~190-200 commandes joueur, interface superficielle par endroits §21 `whatsapp_commands_list.md`). Couche de présentation 100% déterministe (pré-NLU) : après un tour de combat/dialogue/boutique/mouvement/tableau de quêtes, le bot ajoute 1-8 options numérotées déjà résolues par L1 (+ `9` aide universelle, `0` pagination si besoin) ; résolution par citation du message ou chiffre nu (TTL par contexte, `T_PENDING_MENUS`). Ne retire aucune commande texte existante — coexistence totale. | Renforce le C4 (fallback sans IA) : réduit le rappel-mémoire au moment de jouer sans dépendre du NLU/LLM pour fonctionner |
